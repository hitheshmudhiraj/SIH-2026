...
# Ensure backend root is on sys.path so modules like database, models, optimizer, etc. resolve
...
# Import existing operational engines and models from the prototype
...
app = FastAPI(
    ...
    description=(
        "Decision-Support Tool for Indian Railways Maintenance Block Planning. "
        "PROTOTYPE — Simulated Data — Human Approval Required. Not an autonomous control system."
    ),
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Allows frontend dev origins
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)

# 1. Include Foundation Health Routers (Prompt 1/10 requirements)
app.include_router(health_router)
app.include_router(health_router, prefix="/api")

# 2. Existing Operational Endpoints (Preserved completely from previous stage)
